...
export default function ScrollProgress1D({ children, ticksToMax = 24, notchSize = 120, polarity = 1, smooth = 0.85, }) {
    const pTarget = useRef(0);
    const [p, setP] = useState(0);
    const clamp = (v) => THREE.MathUtils.clamp(v, -1, 1);
    const atMin = () => pTarget.current <= -1 + 1e-6;
    const atMax = () => pTarget.current >= 1 - 1e-6;
    const deltaToP = (dyPixels) => {
        const notches = dyPixels / notchSize;
        return (-notches * polarity) / ticksToMax;
    };
    const willConsume = (deltaP) => {
        if (deltaP > 0 && atMax())
            return false;
        if (deltaP < 0 && atMin())
            return false;
    ...
    useEffect(() => {
        const onWheel = (e) => {
            ...
            if (!willConsume(dP))
                return; // pass-through to native scroll
            e.preventDefault();
