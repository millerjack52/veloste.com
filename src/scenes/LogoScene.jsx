...
const LogoScene = () => {
    ...
    return (<div className="logo-wrap">
      ...
      <Canvas className="logo-canvas" dpr={[
            1,
     ...
     camera={{ position: [0, 0, 7], fov: 80 }}>
        <Suspense fallback={null}>
          <Lights />
          <ScrollProgress1D ticksToMax={8} notchSize={60} polarity={1} smooth={0.85}>
            {(p) => (<group position={[0, 0, groupZ]} rotation={[0, p * maxYaw, 0]}>
                <VelosteLogoModel rotation={[0, Math.PI * 1.5, 0]}/>
                <TipCircles p={p} groupWorldZ={groupZ} deadZone={0.15}/>
                <CircleContentOverlay p={p} deadZone={0.15} easePower={1.5} curvePower={1.6}/>
              </group>)}
          </ScrollProgress1D>
        </Suspense>
