...
function TipCircle({ side, p, baseScale = 0.1, pos = { x: 5, y: 0, z: 0 }, worldZOfGroup = 1, deadZone = 0.15, easePower = 1.5, curvePower = 1.6, color = "#ffffff", }) {
    ...
    const scale = baseScale * Math.pow(fillScale / baseScale, Math.pow(easedT, curvePower));
    const x = side === "right" ? -Math.abs(pos.x) : +Math.abs(pos.x);
    return (<Billboard position={[x, pos.y, pos.z]} follow>
      <mesh scale={scale} renderOrder={10}>
        <circleGeometry args={[1, 64]}/>
        <meshBasicMaterial color={color} transparent={false} depthTest={false} depthWrite={false} side={THREE.DoubleSide} toneMapped={false}/>
      </mesh>
    </Billboard>);
}
